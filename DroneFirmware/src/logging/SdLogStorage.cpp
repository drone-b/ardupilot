...
SdLogStorage::BinaryLogRecord SdLogStorage::make_record(const comms::TelemetryFrame& frame)
{
    ...
    record.power_sample_time_us = frame.power_sample.sample_time_us;
    record.power_voltage_v = frame.power_sample.voltage_v;
    record.power_current_a = frame.power_sample.current_a;
    record.power_remaining_ratio = frame.power_sample.remaining_ratio;
    record.power_valid = frame.power_sample.valid ? 1U : 0U;
    record.power_status = static_cast<std::uint8_t>(frame.power_sample.status);
    record.imu_health_last_update_us = frame.imu_health.last_update_us;
    record.imu_health_total_sample_count = frame.imu_health.total_sample_count;
    record.imu_health_total_error_count = frame.imu_health.total_error_count;
    record.imu_health_consecutive_error_count = frame.imu_health.consecutive_error_count;
    record.imu_health_initialized = frame.imu_health.initialized ? 1U : 0U;
    record.imu_health_healthy = frame.imu_health.healthy ? 1U : 0U;
    record.imu_health_last_status = static_cast<std::uint8_t>(frame.imu_health.last_status);
    record.flight_state = static_cast<std::uint8_t>(frame.flight_state);
    record.safety_block_reason = static_cast<std::uint8_t>(frame.safety_block_reason);
    record.safety_allow_motor_output = frame.safety_allow_motor_output;
    record.safety_arming_allowed = frame.safety_arming_allowed;
    record.safety_transition_count = frame.safety_transition_count;
    record.safety_last_transition_us = frame.safety_last_transition_us;
